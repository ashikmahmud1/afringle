...
<div class="page-container">
  ...
  <div class="main-content">
     <h1 class="page-title"> Please Make plan according to your needs and assign access to your members.</h1>
               ...
               <table class="table table-striped table-bordered table-hover dataTables-example" >
                 <thead>
                   <tr>
                         ...
                         <th>Membership Icon(Icon png only)</th>
                         <th>Membership Price ( only in digit USD)</th>

                         <th>Description</th>
                          <th>Membership Settings</th>
                        <th>Action</th>
                    </tr>
                 </thead>
                 <tbody>
                    <?php if(!empty($ListOfPlan)){


                    foreach($ListOfPlan as $list):
                    ?>

                            <tr role="row" class="gradeA">
                             <td><?=$list['plan_name'];?></td>
                             <td><?=$list['plan_time'];?></td>
                             <td><img src="<?=$list['plan_image'];?>"></td>
                             <td><?=$list['plan_price'];?></td>
                            <td><?php $content = $list['plan_description']; echo substr($content, 0, 100); ?> </td>
                             <td><?=$list['plan_services'];?></td>
                            <td style="text-align: center;">
<a  href="<?=base_url('plan/plan-edit?id='.$list['id']);?>"><i class="ui-tooltip fa fa-pencil info" style="font-size: 22px;margin-right: 15px;color: #3f51b5;" data-original-title="Edit"></i></a>
<a  onclick="return confirm('Are you sure remove this? #<?=$list['plan_name'];?>')" href="<?=base_url('plan/plan-delete?id='.$list['id']);?>"><i class="ui-tooltip fa fa-trash-o" style="margin-right: 14px;color: red;font-size: 22px;" data-original-title="Delete"></i></a>
                             </td>
                         </tr>
